...
class FractalFrame(Frame):
    ...
    def body(self):
        ...
        self.can_fractal = Canvas(self, width=plot_height * 1.5, height=plot_height,
                                  cursor="tcross")
        ...
        self.can_fractal.bind('<Button-2>', self.on_right_click)  # Right-click center (MacOS)
        ...
        self.can_fractal.bind("<ButtonRelease-1>", self.on_button_release)  # Left-release
    ...
    def plot(self):
        ...
        self.mandelbrot.plot_image(self._setmode, width, height, zoom, radius, exponent,
                                   zx_off, zy_off, maxiter, theme, shift, cx_off, cy_off)
        self._fractal = ImageTk.PhotoImage(self.mandelbrot.get_image())
        self.can_fractal.create_image(0, 0, image=self._fractal, state="normal", anchor=NW)

        if self._show_axes:
            self.axes(width, height)
        self.can_fractal.update()

        if not self.mandelbrot.get_cancel() and not self._animating:  # If plot wasn't cancelled
            end = time()
    ...
    def axes(self, width, height):
        ...
        tki = [-2.5, -2, -1.5, -1, -.5, .5, 1, 1.5, 2, 2.5]
        tick = 10
        ...
        self.can_fractal.create_text(0 + 40, height / 2 + yoff + 20, text="Re[c] (X)",
                                     fill="gray", anchor='n')
        self.can_fractal.create_line(yline, fill="gray")
        self.can_fractal.create_text(width / 2 - xoff + 20, 0 + 20, text="Im[c] (Y)",
                                     fill="gray", anchor='w')

        for x_axis in tki:
            xoff, yoff = ctop(width, height, x_axis, 0, zxoff, zyoff, zoom)
            yline = [width / 2 - xoff, height / 2 - tick + yoff, width / 2 - xoff,
                     height / 2 + tick + yoff]
            self.can_fractal.create_line(yline, fill="gray")
            self.can_fractal.create_text(width / 2 - xoff,
                                         height / 2 - tick + yoff - 10, text=x_axis * -1,
                                         fill="gray")
        for y_axis in tki:
            xoff, yoff = ctop(width, height, 0, y_axis, zxoff, zyoff, zoom)
            xline = [width / 2 - tick - xoff, height / 2 + yoff, width / 2 + tick - xoff,
                     height / 2 + yoff]
            self.can_fractal.create_line(xline, fill="gray")
            self.can_fractal.create_text(width / 2 - tick - xoff - 10,
                                         height / 2 + yoff, text=y_axis * -1,
                                         fill="gray", anchor='e')
        self.can_fractal.update()
    ...
    def get_coords(self, event):
        ...
        self.__app.set_status(COORDTXT + " Re[c] (X): " + str(zx_coord) \
                              +",  Im[c] (Y): " + str(zy_coord))
    ...
    def on_mouse_wheel(self, event):
        ...
        self.__app.frm_settings.update_settings(zoom=zoom, zxoffset=zx_coord, zyoffset=zy_coord)
        self.__master.update_idletasks()
    ...
    def on_left_click(self, event):
        ...
        if self._leftclickmode == ZOOMIN:
            zoom = zoom * zoominc
            self.__app.frm_settings.update_settings(zoom=zoom, zxoffset=zx_coord,
                                                    zyoffset=zy_coord)
        ...
        if self._leftclickmode == GOJULIA:
            self.__app.frm_settings.update_settings(settype="Julia", zxoffset=0,
                                                    zyoffset=0, cxoffset=zx_coord,
                                                    cyoffset=zy_coord)
    ...
    def on_key_down(self, event):
        ...
        if (event.keysym == 'Left' or event.keysym == 'Right') and self._setmode == JULIA:
            if event.keysym == 'Left':
                self.rotate_julia(.01)
            else:
                self.rotate_julia(-.01)

            self.plot()

    def on_key_release(self, event):  # pylint: disable=W0613
        '''
    ...
    def on_button_down(self, event):
        ...
        if not self._zoom_rect:
            self._zoom_rect = \
                self.can_fractal.create_rectangle(self._xaxis,
                                                  self._yaxis, 1, 1,
    ...
    def on_button_drag(self, event):
        ...
        self.can_fractal.coords(self._zoom_rect, self._x_start, self._y_start, event.x, event.y)

    def on_button_release(self, event):  # pylint: disable=W0613
        '''
        ...
        self.__app.frm_settings.update_settings(zoom=zoom,
                                                zxoffset=zx_coord, zyoffset=zy_coord)
    ...
    def animate(self, animatemode):
        ...
        for i in range(frames):
            self.__app.set_status(FRMTXT + " " + str(i) + " / "
                                  +str(frames) + " ...")
            self.can_fractal.update()
